...
class BulkPhoneTemplate(models.Model):
    ...
    description = models.TextField(blank=True, null=True) # Assuming description can be optional text
    bulk_message_type = models.ForeignKey(BulkMessageType, on_delete=models.CASCADE, db_column='bulk_message_type_id')
    body = models.TextField() # The content of the phone message
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    class Meta:
        db_table = 'bulk_phone_templates'
        app_label = 'app' # Your app's name
