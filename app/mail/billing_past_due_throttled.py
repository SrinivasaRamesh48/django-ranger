...
@shared_task
def send_billing_past_due_throttled_email(data):
    """
    A Celery task to send a service throttled email for non-payment.
        ...
        data (dict): A dictionary containing the statement and subscriber info.
                     Example:
                     {
                         'statement': {
                             ...
                             'items': [
                                 {'description': {'statement_item_type_id': 1, 'description': 'Previous Balance'}, 'amount': 150.00},
    ...
    try:
        ...
        subject = "Uprise Fiber - Service Throttled for Non-Payment"
        ...
        html_message = render_to_string(
            'mail/billing_past_due_throttled.html',
            {'data': data}
        )
        ...
        print(f"Service throttled notice sent to {subscriber_email}")

    except Exception as e:
        # Log any exceptions that occur during email sending.
        print(f"Error sending service throttled notice: {e}")
