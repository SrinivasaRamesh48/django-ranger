...
@shared_task
def send_billing_rate_limit_imminent_email(data):
    """
    A Celery task to warn of imminent service rate-limiting for non-payment.
    ...
    try:
        ...
        subject = "Uprise Fiber - Payment Past Due"
        ...
        html_message = render_to_string(
            'mail/billing_rate_limit_imminent.html',
            {'data': data}
        )
        ...
        print(f"Rate limit imminent notice sent to {subscriber_email}")

    except Exception as e:
        # Log any exceptions that occur during email sending.
        print(f"Error sending rate limit imminent notice: {e}")
